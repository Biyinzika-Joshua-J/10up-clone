...
const MobileNavbar = () => {
  ...
  return (
    <nav className="bg-main py-6 relative lg:hidden block z-[200]">
      <div className="flex justify-between px-6 items-center">
        <div>
          <Link
            href={"/contact"}
            className="text-[#fefefe] text-[14px] uppercase"
          >
            Contact
          </Link>
        </div>
        ...
        <div>
          <Link href={"/"}>
            <div className="bg-main -ml-[55px] h-[80px] w-[80px] -mb-12 rounded-full -top-[1px] absolute flex justify-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 235.84 269.94"
                className="w-[36px] z-50"
              >
      ...
      <div
        className={`bg-main absolute transition-all duration-150 ${
          open ? "h-[230px]" : "h-[0px] overflow-hidden"
        } w-full`}
      >
        <ul className="flex flex-col justify-center items-center pt-10">
          {navLinks.map(({ text, path }) => (
            <li
              key={text}
              className="mx-6 tracking-[.15rem] my-2 uppercase text-[12px] font-[600]"
            >
              <Link href={path} className={`text-[#fefefe]`}>
                {text}
              </Link>
            </li>
          ))}
